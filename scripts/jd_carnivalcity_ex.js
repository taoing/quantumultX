...
let context = {};
context.ua = "Mozilla/5.0 (iPhone; CPU iPhone OS 13_7 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.1.2 Mobile/15E148 Safari/604.1";
context.path = "/khc/record/convertCoupon";
context.md5AppKey = "07035cabb557f09a5";
...
!(async () => {
  ...
  for (let i = 0; i < $.cookieArr.length; i++) {
    ...
    if ($.currentCookie) {
      ...
      while (!context.isDone && context.tryTime < 5) {
        await exchange();
        context.tryTime += 1;
      }
    }
...
function exchange() {
  return new Promise(async (resolve) => {
    ...
    let url = {
      url: JD_API_HOST + context.path + "?" + queryStr,
      headers: {
